...
function generateData1() {
	...
	while (i < 6) {
		...
		if(i==0 || i==1)
			data.push({'value' : 0, 'date' : date, 'label':x_label});
		if(i==2 || i==3)
			data.push({'value' : 2, 'date' : date, 'label':x_label});
		if(i==4 || i==5)
			data.push({'value' : 4, 'date' : date, 'label':x_label});
		i++;
	}
	return data;
}
...
{
    var i = 0;
    $( "#chart_small_second text[x='0']" ).each(function( index ) {
        $(this).text(labels[i]);
