		...
		colPosition = 0; // for printing colors on simple bar graph
 		...
 		for(var val in data){
			...
			unique = val+el.id; //unique identifier
 			...
 			out += "<div class='graphValue"+el.id+"' id='graphValue"+unique+"'><input type='hidden' value='Poeple Tracked:"+prefix+value+postfix+" '/>&nbsp;</div>";
 			
 			out += "<div class='graphBar"+el.id+"' id='graphFieldBar"+unique+"' style='background-color:"+color+";position: relative; overflow: hidden;'><input type='hidden' value='"+prefix+value+postfix+" People Tracked "+lbl+"'/></div>";
